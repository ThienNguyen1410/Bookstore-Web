...
<html lang="en" class="no-touchevents flexbox csscolumns csscolumns-width csscolumns-span csscolumns-fill csscolumns-gap csscolumns-rule csscolumns-rulecolor csscolumns-rulestyle csscolumns-rulewidth csscolumns-breakbefore csscolumns-breakafter csscolumns-breakinside smil">
    ...
    <div class="se-pre-con"></div>
     ...
     <div class="profileEditBody">
        ...
        <div class="mainside">
        <form onSubmit="return checkEdit()" action="${pageContext.request.contextPath}/profile_user_edit" method="post">
          <h1><b>Information</b></h1>
          <hr style="background-color: black;">
          <h2>1.Full name</h2>
          <p><input type="text" name="profile_full_name" value ="<% if(cus != null) { out.print(cus.getFullName());} else{out.print("");}%>" readonly>(Old)</p>
          <p><input type="text" name="profile_full_name_edit" placeholder="Your Full Name" value ="" required>(New)</p>
          <h2>2.Address (Optional)</h2>
          <p><input type="text" name="profile_address" value ="<% if(cus != null) { out.print(cus.getAddress());} else{out.print("");}%>" readonly>(Old)</p>
          <p><input type="text" name="profile_address_edit" placeholder="New Address" value ="" required>(New)</p>
          <h2>3.User name</h2>
          <p><input type="text" name="profile_user_name" value ="<% if(cus != null) { out.print(cus.getUserName());} else{out.print("");}%>" readonly>(Old)</p>
          <p><input type="text" name="profile_user_name_edit" placeholder="New User Name" value ="" required>(New)</p>
          <h2>4.Email</h2>
          <p><input type="text" name="profile_email" value ="<% if(cus != null) { out.print(cus.getEmail());} else{out.print("");}%>"readonly>(Old)</p>
          <p><input type="text" name="profile_email_edit" placeholder="New email" value ="" required>(New)</p>
          <h2>5.Password</h2>
          <p><input type="password" name="profile_pass" value ="<% if(cus != null) { out.print(cus.getPass());} else{out.print("");}%>"readonly>(Old)</p>
          <p><input type="password" name="profile_pass_edit" placeholder="New password" value ="" id="profile_pass_edit" required>(New)</p>
          <input name="show_pass_edit"  type="checkbox" onclick="myFunctionShowEdit()"><p id="show_pass_edit" > Show password.</p>
          
          <p><input type="submit" name="edit_profile" value="Complete"></p>
        </form>
        </div> 
      </div> 
      <div class="footer">
